...
"""
    ...
    fun dummyData(): ArrayList<Earthquake> {
        return parseJsonData(SAMPLE_JSON_RESPONSE)
    }
    /**
     * Return a list of [Earthquake] objects that has been built up from
     * parsing a JSON response.
     */
    fun parseJsonData(jsonData: String): ArrayList<Earthquake> {
        val earthquakes = ArrayList<Earthquake>()
        if (jsonData != "") {
            try {
                val root = JSONObject(jsonData)
                val featuresArray: JSONArray = root.get("features") as JSONArray
                for (i in 0..(featuresArray.length() - 1)) {
                    val feature = featuresArray.get(i) as JSONObject
                    ...
                    val dateTime = properties.getLong("time")
                    val url = properties.getString("url")

                    earthquakes.add(Earthquake(mag, location, dateTime, url))
                }
            } catch (e: JSONException) {
                throw java.io.IOException("Error parsing JSON:" + e.localizedMessage)
            }
        }
